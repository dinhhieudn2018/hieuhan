<!-- Header -->
	<div class="logo-w3layouts">
		<div class="header-top-w3ls">
			<div class="container">
				<div class="w3l-social">
					<ul>
						<li>
							<a href="https://www.facebook.com/nhakhoahieuhandanang/" target="_blank" class="fa fa-facebook"></a>
						</li>
						<li>
							<a href="#" class="fa fa-twitter"></a>
						</li>
					</ul>
				</div>
				<div class="w3l-social" style="margin-left: 110px;">
					<h1>
						<a href="#">
							<span style="color: white">Nha Khoa</span>
							<span style="color: white">Hiếu</span>
							<span style="color: white">Hân</span></a>
					</h1>
				</div>
				<div class="right-p">
					<ul>
						<li>
							<span class="fa fa-phone" aria-hidden="true"></span>0905 050 824</li>
						<li>
							<span class="fa fa-clock-o" aria-hidden="true"></span>( Mon-Fri 9am - 8pm ) ( Sat-Sun 10am - 6pm )</li>
					</ul>
				</div>
				<div class="clearfix"></div>
			</div>
		</div>
		
	</div>
	<!-- banner-slider -->
	<div class="w3l_banner_info" id="home">
		<div class="slider">
			<div class="callbacks_container">
				<!-- Navigation -->
				<div class="header-nav">
					<div class="container">
						<nav class="navbar navbar-default">
							<div class="navbar-header logo">
								<button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1">
									<span class="sr-only">Toggle navigation</span>
									<span class="icon-bar"></span>
									<span class="icon-bar"></span>
									<span class="icon-bar"></span>
								</button>
							</div>
							<!-- navbar-header -->
							<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
								<ul class="nav navbar-nav navbar-right">
									<li>
										<a href="index.html" class="active scroll">Trang chủ</a>
									</li>
									<li>
										<a href="#price-table" class="scroll">Bảng giá</a>
									</li>
									<li>
										<a href="#booking" class="scroll">Đặt hẹn - Tư vấn</a>
									</li>
									<li>
										<a href="#image" class="scroll">Hình ảnh</a>
									</li>
									<li>
										<a href="#contact" class="scroll">Liên hệ</a>
									</li>
								</ul>
							</div>
							<div class="clearfix"> </div>
						</nav>
						<div class="clearfix"> </div>
					</div>
				</div>
				<!-- //Navigation -->
				<ul class="rslides" id="slider3">
					<li>
						<div class="slider-img b1">
						</div>
						<div class="slider_banner_info">
							<div class="w3ls-info">
								<h4>
									<span>T</span>ẩy
									<span>Tr</span>ắng</h4>
								<p>
									Chúng tôi có thể cải thiện sức khỏe răng miệng của con bạn.
								</p>
							</div>
						</div>
					</li>
					<li>
						<div class="slider-img b2">
						</div>
						<div class="slider_banner_info">
							<div class="w3ls-info">
								<h4>
									<span>Đ</span>iều
									<span>Trị</span> Tận Gốc</h4>
								<p>Chúng tôi rất am hiểu sức khỏe răng miệng của con bạn.</p>
							</div>
						</div>
					</li>
					<li>
						<div class="slider-img b3">
						</div>
						<div class="slider_banner_info">
							<div class="w3ls-info">
								<h4>
									<span>C</span>ác
									<span>Dịch vụ</span>Khác</h4>
								<p>Chúng tôi sẽ giúp bạn hài lòng.</p>
							</div>
						</div>
					</li>
				</ul>
			</div>
		</div>
	</div>
	<div class="clearfix"></div>
	<!-- //banner-slider -->
	<!-- //Header -->